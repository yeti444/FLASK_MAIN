...
def get_all_ScheduledMaintenance():
    ...
    cur.close()
    conn.close()
    ScheduledWork_entries = [ScheduledMaintenance(maintId = entry[0], userId = entry[1], fromDate = entry[2], duration = entry[3], createDate = entry[4]) for entry in data]
...
def get_one_ScheduledMaintenance(maintId):
    ...
    if ScheduledMaintenance:
        return ScheduledMaintenance(maintId = ScheduledMaintenance_entry[0], userId = ScheduledMaintenance_entry[1], fromDate = ScheduledMaintenance_entry[2], duration = ScheduledMaintenance_entry[3], createDate = ScheduledMaintenance_entry[4])
    else: 
        None

def create_ScheduledMaintenance(userId, fromDate, duration):
    conn = db_conn()
    cur = conn.cursor()
    cur.execute('''insert into scheduledmaintenance (userId, fromDate, duration) values ('{0}', '{1}', '{2}') returning maintId;'''.format(userId, fromDate, duration))
